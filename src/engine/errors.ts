...
// with status 0. Phase 0 records the exact text and tightens this if needed.
const CORS = /NetworkError|Failed to fetch|CORS|HTTP (?:status )?0\b|status(?: code)? 0\b/i;
const MEMORY = /Out of Memory|could not allocate|failed to allocate|memory_limit/i;
